...
type incdecop = PreInc | PostInc | PreDec | PostDec [@@deriving show, ord, eq]
...
and 'a expr_node =
  ...
  | IncDec of 'a lvalue * incdecop (* Pre/Post increment/decrement operations *)
...
and 'a stmt_node =
  ...
  | For of 'a expr option * 'a expr option * 'a expr option * 'a stmt
...
(* Creates a new annotaded_node *)
let make_node _node _annot = { node = _node; annot = _annot }

(* Auxilary type for top_decl *)
type 'a top_decl_aux = 
  | Interface of 'a interface_decl
  | Component of 'a component_decl
  | Connection of connection
  | ConnectionList of connection list
[@@deriving show, ord, eq]
